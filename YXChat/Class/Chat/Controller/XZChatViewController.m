...
@interface XZChatViewController ()<ICChatBoxViewControllerDelegate,UITableViewDelegate,UITableViewDataSource,ICRecordManagerDelegate,UIViewControllerTransitioningDelegate,UIViewControllerAnimatedTransitioning,BaseCellDelegate,JMessageDelegate>
{
    CGRect _smallRect;
    CGRect _bigRect;
...
@property (nonatomic,assign) NSInteger jMessageCount;
...
- (void)loadDataSource
{
    ...
    NSArray<JMSGMessage *> *messageArray = [self.conversation messageArrayFromNewestWithOffset:[NSNumber numberWithInteger:self.jMessageCount] limit:[NSNumber numberWithInteger:18]];
    for (int i = messageArray.count - 1 ; i >= 0; i --) {
        JMSGMessage *message = messageArray[i];
        if (message.contentType == kJMSGContentTypeText) {
            ...
            if (message.isReceived) {
                ICMessageFrame *messageF = [ICMessageHelper createMessageFrame:TypeText content:msgText path:nil from:@"gxz" to:self.group.gId fileKey:nil isSender:NO receivedSenderByYourself:NO];
...
//                [self addObject:messageF isSender:YES];
                ...
                ICMessageFrame *messageF = [ICMessageHelper createMessageFrame:TypeText content:msgText path:nil from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO];
                messageF.model.message.deliveryState = ICMessageDeliveryState_Delivered;
//                [self addObject:messageF isSender:YES];
                [self.dataSource addObject:messageF];
            }
...
- (void)sendTextMessageWithContent:(NSString *)messageStr
{
    ...
    JMSGTextContent *content = [[JMSGTextContent alloc] initWithText:messageStr];
    JMSGMessage *jmsgmessage = [JMSGMessage createSingleMessageWithContent:content username:[JMSGUser myInfo].username];
    JMSGOptionalContent *optionalContent = [[JMSGOptionalContent alloc] init];
    optionalContent.needReadReceipt = NO;
    [self.conversation sendMessage:jmsgmessage optionalContent:optionalContent];
